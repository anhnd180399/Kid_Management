...
function onEachUserConnection(socket) {
	...
	print('from parent user:  ' + from_user_id);
	// Add to Map
	let userMapVal = { socket_id: socket.id };
	if (socket.handshake.query.parent == 'true'){
		addUserToMap(from_user_id, userMapVal);

	}else{
		addKidToMap(from_user_id, userMapVal);
	}
	print(userMap);
	printNumOnlineUsers();
	printNumOnlineKid();
...
function singleChatHandler(socket, chat_message) {
	...
	print(from_user_id + '=> KID');
	let kidIds = Array.from(kidMap.keys());
	kidIds.forEach(element => {
		print('kid id' + element);
		let to_user_socket_id = getKidSocketIDfromMapForKid(element);
		let userOnline = kidFoundOnMap(element);
		print('to_user_socket_id: ' + to_user_socket_id + ', userOnline: ' + userOnline);
		print('data: ' + JSON.stringify(chat_message.message));
		if (!userOnline) {
...
function checkOnlineHandler(socket, chat_user_data) {
	...
	let user_online = kidFoundOnMap(to_user_id);
...
function onDisconnect(socket) {
	socket.on(ON_DISCONNECT, function () {
		...
		removeKidWithSocketIdFromMap(socket.id);
...
// kid
function addKidToMap(key_user_id, val) {
	kidMap.set(key_user_id, val);
}
function removeKidWithSocketIdFromMap(socket_id) {
	print('Deleting user with socket id: ' + socket_id);
	let toDeleteUser;
	for (let key of kidMap) {
		// index 1, returns the value for each map key
		let userMapValue = key[1];
		if (userMapValue.socket_id == socket_id) {
			toDeleteUser = key[0];
	...
	print('Deleting User: ' + toDeleteUser);
	if (undefined != toDeleteUser) {
		kidMap.delete(toDeleteUser);
	}
	print('Kid Maps: ');
	print(kidMap);
	printNumOnlineKid();
}
function printNumOnlineKid() {
	print('Online Kids: ' + kidMap.size);
}
function kidFoundOnMap(to_user_id) {
	let to_user_socket_id = getKidSocketIDfromMapForKid(to_user_id);
	return to_user_socket_id != undefined;
}
function getKidSocketIDfromMapForKid(to_user_id) {
	let userMapVal = kidMap.get(`${to_user_id}`);
	if (userMapVal == undefined) {
		return undefined;
	}
	return userMapVal.socket_id;
